...
use crate::{structs::register_user::RegisterUser, utils::register::{generate_token, send_html_message}, AppState};
...
pub async fn register_route(
    ...
    let email = format!("{}|{}", email_confirm_token, register_user.email.to_lowercase());

    match match sqlx::query!("SELECT * FROM users WHERE email = $1", email).fetch_optional(&app_state.pool).await {
        Ok(result) => result,
        Err(e) => {
    ...
    match sqlx::query!("INSERT INTO email_confirm (email, token) VALUES ($1, $2);", email, email_confirm_token).execute(&app_state.pool).await {
        Ok(_) => (),
        Err(e) => {
            info!("{e}");
            return (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()).into_response();
    ...
    match send_html_message(app_state.smtp_client, "Confirm email", &format!("<h1>Token : {}</h1>", email_confirm_token), register_user.email.parse().unwrap()) {
        Ok(_) => (),
        Err(e) => {
